...
export default async function AdminOverviewPage() {
  ...
  const statusItems = Object.entries(stats.playbooksByStatus)
    .sort((a, b) => b[1] - a[1])
    .map(([label, value]) => ({ label: label.replace(/_/g, ' '), value, color: STATUS_COLORS[label] ?? '#a1a1aa' }))

  return (
    <div className="space-y-8">
      <div>
        <h1 className="font-heading text-2xl font-bold text-white">Overview</h1>
        <p className="text-sm text-[#a1a1aa] mt-0.5">Platform-wide health, usage, and revenue.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card className="bg-[#141419] border-white/[0.06] p-5">
          <TrendChart label="Playbooks created (30d)" data={timeseries.map((d) => ({ date: d.date, value: d.playbooks }))} color="#339af0" />
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card className="bg-[#141419] border-white/[0.06] p-5">
          ...
          {statusItems.length ? <DistributionBars items={statusItems} /> : <p className="text-xs text-[#a1a1aa]">No playbooks yet.</p>}
        </Card>
        <Card className="bg-[#141419] border-white/[0.06] p-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card className="bg-[#141419] border-white/[0.06] overflow-hidden">
          ...
          {failed.length === 0 ? (
            <p className="text-sm text-[#a1a1aa] px-5 py-8 text-center">No failures. 🎉</p>
          ) : (
        ...
        <Card className="bg-[#141419] border-white/[0.06] overflow-hidden">
          ...
          {signups.length === 0 ? (
            <p className="text-sm text-[#a1a1aa] px-5 py-8 text-center">No users yet.</p>
          ) : (
            <div className="divide-y divide-white/[0.04]">
              {signups.map((u) => (
                <Link key={u.id} href={`/admin/users/${u.id}`} className="block px-5 py-3 hover:bg-white/[0.02] transition-colors">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="text-right flex-shrink-0">
                      ...
                      <p className="text-[10px] text-[#a1a1aa]">{formatDistanceToNow(u.createdAt, { addSuffix: true })}</p>
                    </div>
                  </div>
                </Link>
              ))}
