...
const RUN_STATUS_COLORS: Record<string, string> = {
  ...
  queued: 'text-[#a1a1aa] bg-white/5 border-white/15',
...
export default async function AdminPlaybookDetailPage({
  ...
  return (
    <div className="space-y-6">
      <Link href="/admin/playbooks" className="inline-flex items-center gap-1.5 text-xs text-[#a1a1aa] hover:text-white">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <div className="flex flex-wrap items-center gap-3 mt-1.5 text-xs text-[#a1a1aa]">
            {pb.user && (
      ...
      {pb.failedReason && (
        <div className="flex items-start gap-3 p-4 rounded-xl bg-red-500/5 border border-red-500/20">
          ...
          <div>
            ...
            <p className="text-sm text-[#a1a1aa] mt-0.5">{pb.failedReason}</p>
          </div>
        </div>
      )}
              ...
              <div key={r.id} className="px-5 py-3">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-[#a1a1aa]">attempt {r.attempt}</span>
                  </div>
                  <span className={`text-[10px] font-medium px-2 py-0.5 rounded border capitalize ${RUN_STATUS_COLORS[r.status] ?? 'text-[#a1a1aa] bg-white/5 border-white/15'}`}>
                    {r.status}
                  </span>
                </div>
                <div className="flex flex-wrap gap-x-4 gap-y-0.5 mt-1 text-[11px] text-[#a1a1aa]">
              ...
              <div key={c.id} className="px-5 py-3">
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-white">{c.name} <span className="text-[#a1a1aa] font-normal">· {c.title}</span></p>
                    <div className="flex flex-wrap items-center gap-3 mt-1 text-[11px] text-[#a1a1aa]">
                      {c.email && <span className="flex items-center gap-1"><Mail className="w-3 h-3" />{c.email}</span>}
                      {c.linkedinUrl && (
                        <a href={c.linkedinUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[#339af0] hover:underline">
                  ...
                  <span className={`text-[10px] font-medium px-2 py-0.5 rounded border capitalize ${CONTACT_STATUS_COLORS[c.verificationStatus] ?? 'text-[#a1a1aa] bg-white/5 border-white/15'}`}>
                    {c.verificationStatus.replace(/_/g, ' ')}
                  </span>
                </div>
                {c.rationale && <p className="mt-1.5 text-[11px] text-[#a1a1aa]">{c.rationale}</p>}
              </div>
            ))}
          </div>
        )}
            ...
            {pb.sections.map((s) => (
              <div key={s.id} className="px-5 py-4">
                <div className="flex items-center justify-between gap-3 mb-2">
                  ...
                  <span className="text-[10px] text-[#a1a1aa] capitalize">{s.status}</span>
                </div>
                {s.contentMarkdown?.trim() ? (
                  <div
                    className="prose prose-invert prose-sm max-w-none text-[#a1a1aa] prose-headings:text-white prose-strong:text-white prose-a:text-[#339af0]"
                    dangerouslySetInnerHTML={{ __html: marked.parse(s.contentMarkdown, { async: false }) as string }}
                  />
                ) : (
                  <p className="text-[11px] text-[#a1a1aa]/60 italic">Empty.</p>
                )}
              </div>
            ))}
          </div>
          ...
          <div className="divide-y divide-white/[0.04] max-h-96 overflow-y-auto">
            {pb.events.map((e) => (
              <div key={e.id} className="px-5 py-2 flex items-start gap-3">
                <span className="text-[10px] text-[#a1a1aa] font-mono flex-shrink-0 w-32">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-[#a1a1aa] break-words">{e.message}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {pb.sources.length > 0 && (
        <Section title={`Sources (${pb.sources.length})`}>
          <div className="divide-y divide-white/[0.04]">
            {pb.sources.map((src) => (
              <a key={src.id} href={src.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-5 py-2 text-xs text-[#339af0] hover:bg-white/[0.02] transition-colors">
...
function Meta({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div>
      <p className="text-[10px] text-[#a1a1aa] uppercase tracking-wider mb-0.5">{label}</p>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      ...
      <Card className="bg-[#141419] border-white/[0.06] overflow-hidden">{children}</Card>
    </div>
  )
}

function Empty({ children }: { children: React.ReactNode }) {
  return <p className="text-sm text-[#a1a1aa] px-5 py-8 text-center">{children}</p>
}
