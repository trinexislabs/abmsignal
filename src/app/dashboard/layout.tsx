...
function UserAvatar({ image, name, size = 'sm' }: { image?: string | null; name?: string | null; size?: 'sm' | 'md' }) {
  ...
  return (
    <Avatar className={cn(dim, 'flex-shrink-0')}>
      ...
      <AvatarFallback className={cn('bg-[#1e3a5f] text-[#339af0] font-bold', textSize)}>
        {initials}
      </AvatarFallback>
    </Avatar>
  )
    ...
    >
      ...
      <div className={cn('flex items-center h-16 px-4 border-b border-white/[0.06] flex-shrink-0', collapsed ? 'justify-center' : 'justify-between')}>
        {!collapsed ? (
          <>
            ...
            <button onClick={onCollapse} className="p-1.5 rounded-md text-[#a1a1aa] hover:text-white hover:bg-white/5 transition-colors">
      ...
      <div className={cn('px-2 pt-4 pb-2', collapsed && 'flex justify-center')}>
        <Link href="/playbook/new/product" className={collapsed ? '' : 'block'}>
          {collapsed ? (
            <button className="w-10 h-10 rounded-lg bg-[#339af0] hover:bg-[#339af0]/90 flex items-center justify-center transition-colors" title="New Playbook">
      ...
      <div className="px-2 py-4 border-t border-white/[0.06] flex-shrink-0">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" side="top" className="w-52 bg-[#141419] border-white/10">
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} onClick={onClose}>
                <div className={cn('flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all', isActive ? 'bg-[#1e3a5f] text-white' : 'text-[#a1a1aa] hover:text-white hover:bg-white/5')}>
    ...
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <div className={cn('transition-all duration-300', collapsed ? 'lg:ml-16' : 'lg:ml-60')}>
        ...
        <header className="h-16 border-b border-white/[0.06] bg-[#0a0a0f]/80 backdrop-blur-xl flex items-center justify-between px-4 sm:px-6 sticky top-0 z-30">
          <button className="lg:hidden p-1.5 text-[#a1a1aa] hover:text-white transition-colors" onClick={() => setMobileOpen(true)}>
          ...
          <div className="flex items-center gap-2">
            <button className="p-2 rounded-lg text-[#a1a1aa] hover:text-white hover:bg-white/5 transition-colors">
              <Bell className="w-4 h-4" />
            </button>
            <Link href="/playbook/new/product" className="hidden sm:block">
              <Button size="sm" className="bg-[#339af0] hover:bg-[#339af0]/90 text-white text-xs font-medium gap-1.5 h-8 px-3">
                <Plus className="w-3.5 h-3.5" />
                New Playbook
              </Button>
            </Link>
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-48 bg-[#141419] border-white/10">
                <div className="px-2 py-1.5">
                  <p className="text-sm font-medium text-white truncate">{user?.name ?? 'User'}</p>
